...
| `AutoRun-fc-scale.txt` | The toggle above **plus the camera's own EL Zone scale** on a third state: press 1 on, press 2 on with the scale across the bottom, press 3 off. Bands, colours and label glyphs are the firmware's own tables (`0xC0D1B74C`, palette `0xC0D1B34C`), rendered by `tools/verify_fcscale.py` to `builds/fcscale/scale.png`. | As above | **Confirmed on a camera** (2026-09-20, after a battery pull). Drawn on the 16-bit main OSD layer inside the display submit (`0xC02E8A08` hook), once per frame; a press paints the front buffer directly. Also as `fp-fc-scale-card.zip`. **Pull the battery when changing AutoRun builds**: a soft power cycle keeps the previous build resident. |
