...
|---|---|---|---|
| `AutoRun-menu.txt` | **The menu card.** Every extra recording mode in one on-camera menu: press **RIGHT** to move, **UP** to turn one on, **Stock** to clear everything. Needs `VSHL.BIN` beside it on the card. | On camera, then re-latch the mode | **Start here.** No USB shell, no diagnostics. |
| `AutoRun-menu-dev.txt` | The same menu **plus the fpSup USB shell** and the `SEL` row's cell counts, for working on the patches over USB. Needs `VSHL.BIN`. | As above | For development. The shell replaces the camera's normal PTP interface, so SIGMA Camera Control will not see the camera. |
